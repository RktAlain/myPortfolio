...
import { Brain, Database, Code, TrendingUp, Mail, Phone, Linkedin, MapPin, Home, Globe, Briefcase } from 'lucide-react';
...
const About = () => {
  ...
  return (
    <section id="about" className="py-20 px-4 bg-[radial-gradient(ellipse_at_top,_var(--tw-gradient-stops))] from-purple-900/30 via-slate-900 to-slate-950">
            ...
            <div className="relative group perspective-1000 flex-grow">
              <div className="relative w-full h-full transition-all duration-700 transform-style-preserve-3d group-hover:rotate-y-10 group-hover:rotate-x-2">
                <div className="bg-gradient-to-br from-slate-800/90 to-slate-900/90 rounded-3xl border border-white/10 shadow-2xl p-8 h-full flex flex-col backdrop-blur-sm">
                  ...
                  <h3 className="text-2xl font-bold text-white text-center mb-2">Solohery Alain</h3>
                  <p className="text-purple-300 text-center mb-6">Data Scientist & Développeur Fullstack</p>
                  
                  <div className="flex justify-center space-x-4 mt-auto">
                    <a href="tel:+261346697437" className="w-12 h-12 rounded-full bg-white/5 border border-white/10 flex items-center justify-center hover:bg-purple-500/20 transition-all">
                      <Phone className="h-5 w-5 text-white" />
                    </a>
                    <a href="mailto:rakotomalalasoheryalain@gmail.com" className="w-12 h-12 rounded-full bg-white/5 border border-white/10 flex items-center justify-center hover:bg-blue-500/20 transition-all">
                      <Mail className="h-5 w-5 text-white" />
                    </a>
                    <a href="https://linkedin.com/in/solohery-alain" target="_blank" className="w-12 h-12 rounded-full bg-white/5 border border-white/10 flex items-center justify-center hover:bg-cyan-500/20 transition-all">
              ...
              <ul className="space-y-3">
                {availabilityItems.map((item, index) => (
                  <li key={index} className="flex items-start text-gray-300">
                    <item.icon className={`h-5 w-5 mr-3 mt-0.5 ${item.highlight ? 'text-green-400' : 'text-purple-300'}`} />
                    <span className={item.highlight ? "text-white font-medium" : ""}>{item.text}</span>
                  </li>
                ))}
              </ul>
            </div>
          ...
          <div className="flex flex-col gap-8 h-full">
            ...
            <div className="bg-white/5 border border-white/10 rounded-3xl p-8 backdrop-blur-sm hover:backdrop-blur-md transition-all duration-500 shadow-lg">
              ...
              <p className="text-gray-300 leading-relaxed">
                Passionné par l'innovation technologique, je combine expertise en analyse de données et développement fullstack 
                pour créer des solutions digitales performantes et intuitives.
              </p>
            </div>
